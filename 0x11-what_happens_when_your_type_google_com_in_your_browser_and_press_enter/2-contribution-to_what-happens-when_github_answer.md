...
When the user presses the "g" key in their browser's address bar, several processes are triggered within the browser and the operating system. This section explores the physical keyboard actions and the OS interrupts that occur.

Upon pressing the "g" key, the browser receives the corresponding keyboard event, and the auto-complete functions are initiated. The browser's algorithm, which might take into account browsing history, bookmarks, cookies, and popular internet searches, starts suggesting possible results in the dropdown beneath the URL bar. These suggestions refine with each subsequent keypress, and they might even predict the input "google.com" before the user completes typing it.

### The "enter" key bottoms out

After typing "google.com" and completing the input, the user presses the "enter" key. This action has significant consequences in the journey of the browser's interaction with the system.

When the "enter" key is pressed, an electrical circuit specific to the "enter" key is closed, allowing a small amount of current to flow into the logic circuitry of the keyboard. This circuitry scans the state of each key switch, eliminates electrical noise from the rapid intermittent closure of the switch, and converts it into a keycode integer. For the "enter" key, this keycode is usually 13. The keyboard controller then encodes this keycode for transport to the computer.

In the case of a USB keyboard, the keyboard's USB circuitry is powered by the 5V supply from the computer's USB host controller. The generated keycode is stored in the internal keyboard circuitry memory in a register called "endpoint." The host USB controller polls this "endpoint" every ~10ms, extracting the keycode value stored on it. This value is then converted into one or more USB packets adhering to the low-level USB protocol. These packets are transmitted as a differential electrical signal over D+ and D- pins, at a maximum speed of 1.5 Mb/s, since an HID (Human Interface Device) is categorized as a "low-speed device" under USB 2.0 compliance.

For touch screen devices with virtual keyboards, the process is different. When the user touches the screen, a minute current is transferred to their finger, completing a circuit through the electrostatic field of the conductive layer. This generates a voltage drop at the corresponding point on the screen. This touch event is then relayed to the OS, triggering the on-screen virtual keyboard application.

### Interrupt fires [NOT for USB keyboards]

For traditional keyboards that connect via PS/2 or ADB interfaces, the keyboard sends signals on its interrupt request line (IRQ). The interrupt controller maps these signals to interrupt vectors (integers), which the CPU indexes in the Interrupt Descriptor Table (IDT). The IDT contains functions (interrupt handlers) provided by the kernel. When an interrupt arrives, the appropriate handler is executed, and the kernel is invoked.

### (On Windows) A WM_KEYDOWN message is sent to the app

When using Windows, the HID transport transfers the key down event to the KBDHID.sys driver. This driver converts the HID usage into a scancode (e.g., VK_RETURN for "enter" key). The KBDHID.sys driver interacts with the KBDCLASS.sys (keyboard class driver), responsible for secure keyboard input handling. The message is then passed to Win32K.sys, which eventually uses the SendMessage API to send a WM_KEYDOWN message to the active window. The window's message processing function (WindowProc) handles this message and detects that the "enter" key has been pressed.
